...
class JSonDteComplementariosComercialesService {
    ...
    public generateDatosComercialesUsoGeneral(params: any, data: any) {
        ...
        if (data['tipoDocumento'] == 1 || data['tipoDocumento'] == 7) { //Opcional si 1 o 7
            if (data['complementarios'] && data['complementarios']['carga'] && data['complementarios']['carga']['volumenTotal'] || 
    ...
    private generateDatosCarga(params: any, data: any) {
        ...
        if (data['complementarios'] && data['complementarios']['carga'] && data['complementarios']['carga']['unidadMedidaVolumenTotal']) {
            if (constanteService.unidadesMedidas.filter(um => um.codigo === data['complementarios']['carga']['unidadMedidaVolumenTotal']).length == 0){
                throw new Error("Unidad de Medida '" + data['complementarios']['carga']['unidadMedidaVolumenTotal'] + "' en data.complementarios.carga.unidadMedidaVolumenTotal no válido. Valores: " + constanteService.unidadesMedidas.map(a=>a.codigo + '-' + a.descripcion));
            }
            jsonResult['cUniMedTotVol'] = data['complementarios']['carga']['unidadMedidaVolumenTotal'];
            jsonResult['dDesUniMedTotVol'] = constanteService.unidadesMedidas.filter(td => td.codigo == data['complementarios']['carga']['unidadMedidaVolumenTotal'])[0]['representacion'];
        }
        if (data['complementarios'] && data['complementarios']['carga'] && data['complementarios']['carga']['volumenTotal']) {
            jsonResult['dTotVolMerc'] = data['complementarios']['carga']['volumenTotal'];
        }
        if (data['complementarios'] && data['complementarios']['carga'] && data['complementarios']['carga']['unidadMedidaPesoTotal']) {
            if (constanteService.unidadesMedidas.filter(um => um.codigo === data['complementarios']['carga']['unidadMedidaPesoTotal']).length == 0){
                throw new Error("Unidad de Medida '" + data['complementarios']['carga']['unidadMedidaPesoTotal'] + "' en data.complementarios.carga.unidadMedidaPesoTotal no válido. Valores: " + constanteService.unidadesMedidas.map(a=>a.codigo + '-' + a.descripcion));
            }
            jsonResult['cUniMedTotPes'] = data['complementarios']['carga']['unidadMedidaPesoTotal'];
            jsonResult['dDesUniMedTotPes'] = constanteService.unidadesMedidas.filter(td => td.codigo == data['complementarios']['carga']['unidadMedidaPesoTotal'])[0]['representacion'];
        }
        ...
        if (data['complementarios'] && data['complementarios']['carga'] && data['complementarios']['carga']['caracteristicaCarga']) {
            if (constanteService.caracteristicasCargas.filter(um => um.codigo === data['complementarios']['carga']['caracteristicaCarga']).length == 0){
                throw new Error("Característica de Carga '" + data['complementarios']['carga']['caracteristicaCarga'] + "' en data.complementarios.carga.caracteristicaCarga no válido. Valores: " + constanteService.caracteristicasCargas.map(a=>a.codigo + '-' + a.descripcion));
            }
            jsonResult['iCarCarga'] = data['complementarios']['carga']['caracteristicaCarga'];  
            jsonResult['dDesCarCarga'] = constanteService.caracteristicasCargas.filter(td => td.codigo == data['complementarios']['carga']['caracteristicaCarga'])[0]['descripcion'];
            ...
            if (data['complementarios']['carga']['caracteristicaCarga'] == 3) {
                if (data['complementarios']['carga']['caracteristicaCargaDescripcion']) {
                    ...
                    throw new Error("Para data.complementarios.carga.caracteristicaCarga = 3 debe informar el campo data.complementarios.carga.caracteristicaCargaDescripcion");
