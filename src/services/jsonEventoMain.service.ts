...
class JSonEventoMainService {
    ...
    private eventosReceptorNotificacion(params: any, data: any) {
        ...
        if (data['iTipRec'] == 2) {
            if (constanteService.tiposDocumentosIdentidades.filter((um:any) => um.codigo === data['dTipIDRec']).length == 0){
                throw new Error("Tipoo de Documento '" + data['dTipIDRec'] + "' en data.dTipIdRec no encontrado. Valores: " + constanteService.tiposDocumentosIdentidades.map((a:any)=>a.codigo + '-' + a.descripcion));
            }
    ...
    private eventosReceptorConformidad(params: any, data: any) {
        ...
        if (constanteService.eventoConformidadTipo.filter((um:any) => um.codigo === data['iTipConf']).length == 0){
            throw new Error("Tipoo de Documento '" + data['iTipConf'] + "' en data.iTipConf no encontrado. Valores: " + constanteService.eventoConformidadTipo.map((a:any)=>a.codigo + '-' + a.descripcion));
        }
